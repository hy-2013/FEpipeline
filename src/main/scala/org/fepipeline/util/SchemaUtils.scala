...
/**
  ...
  */
private[fepipeline] object SchemaUtils extends Logging {
  ...
  def checkNumericTypeAndCast2DoubleType(dataset: Dataset[_],
                                         colName: String): Dataset[_] = {
    val inputColDataType = dataset.schema(colName).dataType
    if (inputColDataType.isInstanceOf[NumericType]) {
      dataset
    } else {
      logWarning(s"NOTE: $colName is not instance of NumericType.")
      dataset.withColumn(s"${colName}Tmp", dataset.col(colName).cast(DoubleType)).drop(colName).withColumnRenamed(s"${colName}Tmp", colName)
    }
  }

  def multiCheckNumericTypeAndCast2DoubleType(dataset: Dataset[_],
                                              colNames: Seq[String]): Dataset[_] = {
    val fields = dataset.schema(colNames.toSet)
    var datasetTmp = dataset
    fields.foreach { fieldSchema =>
      val dataType = fieldSchema.dataType
      val colName = fieldSchema.name
      if (!dataType.isInstanceOf[NumericType]) {
        logWarning(s"NOTE: $colName is not instance of NumericType.")
        datasetTmp = datasetTmp.withColumn(s"${colName}Tmp", dataset.col(colName).cast(DoubleType)).drop(colName).withColumnRenamed(s"${colName}Tmp", colName)
      }
    }
    datasetTmp
  }
